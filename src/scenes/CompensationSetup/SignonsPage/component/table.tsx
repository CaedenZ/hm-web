        ...
        return (
            <Paper className={classes.root}>
                <Table className={classes.table}>
                    ...
                    {this.props.signonsList.length > 0 && <TableBody>
                        {this.props.signonsList.map((row, index) => (
                            <TableRow className={classes.row} key={row.signons_breakdown}>
                                ...
                                <CustomTableCell align="left">{row.signons_breakdown}</CustomTableCell>
                                <CustomTableCell align="left">
                                    <IconButton onClick={() => this.handleUpdateButtonClick(row)}><UpdateIcon /></IconButton>
                                    <IconButton onClick={() => this.handleDelete(row.signons_id, index)}><DeleteIcon /></IconButton>
                                </CustomTableCell>
                            </TableRow>
                        ))}
                    </TableBody>}
